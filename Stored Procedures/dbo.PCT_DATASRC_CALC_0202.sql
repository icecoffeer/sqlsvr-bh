...
SET ANSI_NULLS ON
GO
create procedure [dbo].[PCT_DATASRC_CALC_0202] (
...
begin
  ...
  set @vSql = N'select @poTotal = -isnull(sum(d.TOTAL), 0)'
    + ' from STKINBCK m(nolock), STKINBCKDTL d(nolock)'
    + ' where m.CLS = ''自营'' and m.FILDATE >= @piBeginDate and m.FILDATE < @piEndDate + 1'
