...
SET ANSI_NULLS ON
GO
create procedure [dbo].[RcvoutMRpt] as
begin
  declare
  ...
  begin
    ...
    if @BGDGID is null
    begin
      update NOUTMRPT set NSTAT = 1, NNOTE = '商品不存在'
         where NOUTMRPT.ID = @ID
      fetch next from c_outmrpt into
          @ID,@ASTORE,@ASETTLENO,@BGDGID,@BWRH,@BCSTGID,@DQ1,@DQ2,@DQ3,@DQ4,@DQ5,
          @DQ6,@DQ7,@DT1,@DT2,@DT3,@DT4,@DT5,@DT6,@DT7,@DT91,@DT92,@DI1,@DI2,@DI3,@DI4,
          @DI5,@DI6,@DI7,@DR1,@DR2,@DR3,@DR4,@DR5,@DR6,@DR7/*,@CQ1,@CQ2,@CQ3,@CQ4, @CQ5, @CQ6,
    ...
    if @BCSTGID is null
    begin
       update noutmrpt set nstat=1,nnote = '客户不存在'
          where noutmrpt.id=@id
       fetch next from c_outmrpt into
            @ID,@ASTORE,@ASETTLENO,@BGDGID,@BWRH,@BCSTGID,@DQ1,@DQ2,@DQ3,@DQ4,@DQ5,
            @DQ6,@DQ7,@DT1,@DT2,@DT3,@DT4,@DT5,@DT6,@DT7,@DT91,@DT92,@DI1,@DI2,@DI3,@DI4,
    ...
    (@ASTORE,@ASETTLENO,@BGDGID,@BWRH,@BCSTGID,@DQ1,@DQ2,@DQ3,@DQ4,@DQ5,
     ...
     @CI3,@CI4,@CI5,@CI6,@CI7,@CR1,@CR2,@CR3,@CR4,@CR5,@CR6,@CR7*/) 
    /* 删除NOUTDRPT */
    delete from NOUTMRPT where ID = @ID
    commit transaction
