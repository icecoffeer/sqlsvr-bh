...
SET ANSI_NULLS ON
GO
create procedure [dbo].[edcR_StkOut] (
	@cls	varchar(30),
	@startdate	datetime,
	@finishdate	datetime
)
...
begin
  ...
  if @usergid = @zbgid return 0
	...
	select convert(datetime,convert(varchar(10),o.fildate,102)),'配货出',
	...
	begin
		if @stat = 1 
		    insert into RealExchgDataDtl(Recvdate,cls,num,checkint1,checkint2,tgt,src,checkdata1)
			values (convert(char(10), @fildate, 102),'配货出',@srcnum,@reccnt,@stat,@usergid,@Src,@checkdata1)
		...
		begin
			...
			if @stat = 4
				...
				from stkin o(nolock) where num = @modnum and cls = '配货' and stat = 2 and src = @src
			else
				select @srcnum = o.srcnum,@reccnt = o.reccnt,@modnum = o.modnum
					,@stat = o.stat,@checkdata1= total
				from stkin o(nolock) where Srcnum = @modnum and cls = '配货' and stat = 2 and src = @src
			if @@RowCount = 0
			begin
				select @srcnum = o.srcnum,@reccnt = o.reccnt,@modnum = o.modnum
					,@stat = o.stat,@checkdata1= total
  		    ...
  		    if not exists (select 1 from realexchgdatadtl
			    where recvdate = @fildate and cls= '配货出' 
