...
SET ANSI_NULLS ON
GO
create procedure [dbo].[IncRsvAlcQty] (
  @piStore int,
  @piWrh int,
  @piGdgid int,
  @piQty money,
...
begin
  ...
  select @avlqty = qty - rsvalcqty - @piQty from inv(nolock) where num = @num
  if @avlqty < 0 
  begin
    if (select allowneg from warehouse(nolock) where gid = @piWrh) = 0
    begin
      if @piMode = 0 
        set @piQty = 0 
      else if @pimode =1
        select @piQty = qty - rsvalcqty from inv(nolock) where num = @num
    end
  end
