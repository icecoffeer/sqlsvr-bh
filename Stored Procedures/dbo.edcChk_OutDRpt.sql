...
SET ANSI_NULLS ON
GO
create procedure [dbo].[edcChk_OutDRpt] (
    @SendDate	datetime,
    @src	int,
    @tgt	int,
	@cls	varchar(50),
...
begin
  ...
  if @usergid <> @zbgid return 0
    ...
    select @realint1 = count(adate), 
    	@realdata1 = convert(decimal(20,2),sum(dq1+dq2+dq3+dq4+dq5+dq6+dq7)),
    	@realdata2 = convert(decimal(20,2),sum(dt1+dt2+dt3+dt4+dt5+dt6+dt7))
    from outdrpt(nolock)
