...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[VoucherSellWithStartEndNum]
(
    @num varchar(32),
    @type varchar(64),
    @amount decimal(24, 4),
...
BEGIN
    ...
    -- 发售最后一张
    IF @num IS NOT NULL EXEC VoucherSellSingle @num, @type, @amount, @sellAmount, @storeGid, @operator, @errorMessage OUTPUT;
    IF @errorMessage IS NOT NULL
    BEGIN
      -- 生成时发生了错误
      -- 目前项目上要求发生错误就终止
    ...
    IF @errorCount <> 0
        SET @errorMessage = '成功发售了购物券，但仍有 ' + CONVERT(varchar(16), @errorCount) + ' 张购物券没有处理成功。原因可能是这些购物券不存在，或状态不是已制作。';
    RETURN(0);
END;
GO
