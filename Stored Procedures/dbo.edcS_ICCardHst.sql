...
SET ANSI_NULLS ON
GO
create procedure [dbo].[edcS_ICCardHst] (
	@cls	varchar(30),
	@startdate	datetime,
	@finishdate		datetime
)
...
begin
  ...
  if @usergid = @zbgid return 0
	...
	insert into ShouldExchgDataDtl(senddate,cls,num,checkint1,checkint2,checkint3,checkdata1,checkdata2,checkdata3,tgt,src)
	select convert(datetime,convert(char(10),ih.fildate,102)), 'IC卡记录', ih.cardnum, 
		...
		sum(case ih.action when '转储' then occur else 0 end),@zbgid,@usergid
	from iccardhst ih (nolock)
	where ih.fildate between @startDate and @finishDate
	group by convert(datetime,convert(char(10),ih.fildate,102)),ih.cardnum
end
GO
