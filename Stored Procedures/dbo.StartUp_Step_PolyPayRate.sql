...
SET ANSI_NULLS ON
GO
create procedure [dbo].[StartUp_Step_PolyPayRate]
as
begin
  declare
  ...
  insert into LOG(TIME, MONTHSETTLENO, EMPLOYEECODE, EMPLOYEENAME, WORKSTATIONNO, MODULENAME, TYPE, CONTENT)
    values(@Present, @SettleNo, 'STARTUP', 'HDSVC', 'SETTLEDAY', 'STARTUP', 101, '批量联销率促销单日结终止。' )
  declare c_end cursor for
      select MST.NUM,MST.CLS from POLYPAYRATEPRM MST,POLYPAYRATEPRMDTL DTL
  ...
  begin
    begin transaction
    exec @return_status = POLYPAYRATEPRM_ABORT @Num, @Cls, '日结终止', 1400, @Msg output
    if @return_status <> 0
    begin
      rollback transaction
      begin transaction
      set @Msg = substring('批量联销率促销单 ' + @Num + ' 日结终止失败。'
        + char(10) + @Msg, 1, 255)
