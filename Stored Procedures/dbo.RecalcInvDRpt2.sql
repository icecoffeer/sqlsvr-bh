...
SET ANSI_NULLS ON
GO
create procedure [dbo].[RecalcInvDRpt2]
	@store int,
	@settleno int,
	@date datetime
as
begin
	...
	and astore = @store and bgdgid in (select gdgid from rtlinvtemp)
    ...
    declare c_prc cursor for
        ...
        and m.fildate >= @date and m.fildate <= dateadd(day, -1, @date)  and  gdgid in (select gdgid from rtlinvtemp)
