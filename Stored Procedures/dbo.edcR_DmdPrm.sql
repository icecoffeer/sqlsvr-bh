...
SET ANSI_NULLS ON
GO
create procedure [dbo].[edcR_DmdPrm] (
	@cls	varchar(30),
	@startdate	datetime,
	@finishdate	datetime
)
...
begin
  ...
  if @usergid <> @zbgid return 0
	...
	select convert(datetime,convert(char(10),p.fildate,102)), '促销申请单', p.srcnum, count(pd.line),
		@usergid,p.src,p.stat
	from dmdprm p (nolock), dmdprmdtl pd (nolock)
