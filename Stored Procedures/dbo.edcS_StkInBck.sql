...
SET ANSI_NULLS ON
GO
create procedure [dbo].[edcS_StkInBck] (
	@cls	varchar(30),
	@startdate	datetime,
	@finishdate		datetime
)
...
begin
  ...
  select convert(datetime,convert(varchar(10),o.fildate,102)),
    o.num,o.reccnt,o.total,o.modnum,o.stat,o.vendor
  ...
  begin
    if @stat = 1 
      insert into ShouldExchgDataDtl(SendDate,cls,num,checkint1,checkint2,checkdata1,tgt,src)
        values (convert(char(10), @fildate, 102),'配货进退',@num,@reccnt,@stat,@checkdata1,@tgt,@usergid)
    ...
    begin
	  ...
	  from stkinbck o(nolock) where num = @modnum and stat = 2 and cls = '配货' 
	  if @@Rowcount = 0
	  begin
	    close c_edcS
	    deallocate c_edcS
		raiserror('在配货进货退货单根据修正链找不到原始单据，现在单号是%s',16,1,@num)
		return -1
	  end else
	    insert into ShouldExchgDataDtl(SendDate,cls,num,checkint1,checkint2,checkdata1,tgt,src)
	      values (convert(char(10), @fildate, 102),'配货进退',@num,@reccnt,@stat,@checkdata1,@tgt,@usergid)
