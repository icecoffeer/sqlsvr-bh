...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[VoucherSettleWithStartEndNum]
(
    @num varchar(32),
    @endNum varchar(32),
    @operator varchar(30),
...
BEGIN
    ...
    -- 回收最后一张
    IF @num IS NOT NULL EXEC VoucherSettleSingle @num, @operator, @storeGid, @errorMessage OUTPUT;
    IF @errorMessage IS NOT NULL
    BEGIN
      -- 生成时发生了错误
      -- 目前项目上要求发生错误就终止
    ...
    IF @errorCount <> 0
        SET @errorMessage = '成功对购物券进行了结算，但仍有 ' + CONVERT(varchar(16), @errorCount) + ' 张购物券没有处理成功。原因可能是这些购物券不存在，或状态不是已发放，或已经被结算。';
    RETURN(0);
END;
GO
