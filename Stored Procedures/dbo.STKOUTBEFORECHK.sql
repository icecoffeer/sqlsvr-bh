...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[STKOUTBEFORECHK] (
  @piNum VARCHAR(14),
  @piCls VARCHAR(10),
  ...
  begin
    ...
    declare @CLIENT INT,
            ...
            @CHK VARCHAR(30),
            @Item INT
    select @CLIENT = CLIENT, @TOTAL = TOTAL, @CHECKER = CHECKER from STKOUT where NUM = @piNum and CLS = @piCls
    ...
    insert into CLIENTPRERCVHST (CLIENT, ITEMNO, OPERDATE, AMT, BALANCE, NUM, OPER, CLS) values
      (@CLIENT, @Item, GETDATE(), -1 * @TOTAL, @BALANCE, @piNum, @CHK, @piCls)
    return 0
  end
GO
