...
SET ANSI_NULLS ON
GO
create procedure [dbo].[GetGoodsPrmInPrc]
  @store int, 
  @gdgid int, 
  @curtime datetime,
  @qty money,
...
as begin
  ...
  if @store = @storegid
     select @prm = Promote, @inprc=InPrc from Goods where gid = @gdgid
  else
     select @prm = Promote, @inprc=InPrc from GdStore where storegid = @store and gdgid = @gdgid
  ...
  select @prminprc = @inprc

  declare c_prm cursor for
    select start, finish, cycle, cstart, cfinish, qtylo, qtyhi, inprc 
     from price
     where gdgid = @gdgid and storegid = @store
  open c_prm
  fetch next from c_prm into @start, @finish, @cycle, @cstart, @cfinish, @qtylo, @qtyhi, @inprc 
