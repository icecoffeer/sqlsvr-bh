...
create procedure [dbo].[RcvInYRpt] as
begin
  declare
    ...
    @CR1  MONEY,  @CR2  MONEY,     @CR3  MONEY,      @CR4  MONEY,       @YSD  int
  ...
  begin
    ...
    if @BGDGID is null
    begin
      update NINYRPT set NSTAT = 1, NNOTE = '商品不存在'
         where NINYRPT.ID = @ID
      fetch next from c_inyrpt into
         @ID,@ASTORE,@ASETTLENO,@BGDGID,@BWRH,@BVDRGID,@DQ1,@DQ2,@DQ3,@DQ4,
         @DT1,@DT2,@DT3,@DT4,@DI1,@DI2,@DI3,@DI4,@DR1,@DR2,@DR3,@DR4,@CQ1,@CQ2,@CQ3,
    ...
    if @BVDRGID is null
    begin
       update ninyrpt set  nstat =1,nnote ='供应商不存在'
          where ninyrpt.id=@ID
       fetch next from c_inyrpt into
         @ID,@ASTORE,@ASETTLENO,@BGDGID,@BWRH,@BVDRGID,@DQ1,@DQ2,@DQ3,@DQ4,
         @DT1,@DT2,@DT3,@DT4,@DI1,@DI2,@DI3,@DI4,@DR1,@DR2,@DR3,@DR4,@CQ1,@CQ2,@CQ3,
    ...
    and BGDGID = @BGDGID and BVDRGID=@BVDRGID and bwrh=@bwrh and  ASTORE = @ASTORE )
    begin
      delete from /*2000-3-10 INMRPT*/ INYRPT
    ...
    (@ASTORE,@ASETTLENO,@BGDGID,@BWRH,@BVDRGID,@DQ1,@DQ2,@DQ3,@DQ4,
     ...
     @CR1,@CR2,@CR3,@CR4 )
    /* 删除NINDRPT */
    delete from NINYRPT where ID = @ID
    commit transaction
    fetch next from c_inyrpt into
       @ID,@ASTORE,@ASETTLENO,@BGDGID,@BWRH,@BVDRGID,@DQ1,@DQ2,@DQ3,@DQ4,
       @DT1,@DT2,@DT3,@DT4,@DI1,@DI2,@DI3,@DI4,@DR1,@DR2,@DR3,@DR4,@CQ1,@CQ2,@CQ3,
