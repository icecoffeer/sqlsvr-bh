...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[PURCHASEORDBCK_RCV]
(
  @BILL_ID INT,
  @SRC_ID INT,
  @OPER CHAR(30),
...
BEGIN
  ...
  IF EXISTS(SELECT 1 FROM PURCHASEORDER(NOLOCK) WHERE NUM = @NET_NUM AND CLS = '销售退货' AND STAT = @NET_STAT)
  BEGIN
  ...
  BEGIN
    ...
    BEGIN
      ...
      WHERE SRC = @SRC_ID AND ID = @BILL_ID
      SET @MSG = '本地找不到填单员对应的员工(对照表中也不存在)'
      RETURN 7
    END
  END
  ...
  DELETE FROM PURCHASEORDER WHERE NUM = @NET_NUM AND CLS = '销售退货'
  DELETE FROM PURCHASEORDERDTL WHERE NUM = @NET_NUM AND CLS = '销售退货'
  ...
  INSERT INTO PURCHASEORDER(NUM, CLS, SETTLENO, VENDOR, TOTAL, TAX, NOTE, FILDATE, FILLER,
    ...
    CONFIRMDATE, CONFIRMER, CUSTOMIZEMEMO, BCKCAUSE,
  ...
  INSERT INTO PURCHASEORDERDTL(NUM,CLS, LINE, GDGID, ORDQTY, PRICE, TOTAL, TAX, WRH, INVQTY,
    ARVQTY, BCKQTY, NOTE, FLOWNO, POSNO, RTLQTY, RTLBCKQTY, RTLPRC, RTLTOTAL, PRNUM, ORDPRC, ORDAMT)
  SELECT D.NUM,D.CLS, D.LINE, D.GDGID, D.ORDQTY, D.PRICE, D.TOTAL, D.TAX, G.WRH, D.INVQTY,
    D.ARVQTY, D.BCKQTY, D.NOTE, D.FLOWNO, D.POSNO, D.RTLQTY, D.RTLBCKQTY, D.RTLPRC, D.RTLTOTAL, D.PRNUM, D.ORDPRC, D.ORDAMT
  ...
  WHERE CLS = '销售退货' AND NUM = @NET_NUM
