...
SET ANSI_NULLS ON
GO
create procedure [dbo].[edcS_StkIn] (
	@cls	varchar(30),
	@startdate	datetime,
	@finishdate		datetime
)
  ...
  begin
    ...
    begin
      insert into ShouldExchgDataDtl(SendDate,cls,num,checkint1,checkint2,checkdata1,tgt,src)
        values (convert(char(10), @fildate, 102),'配货进',@num,@reccnt,@stat,@checkdata1,@tgt,@usergid)
    end
    ...
    begin
	  ...
	  from stkin o(nolock) where num = @modnum and stat = 2 and cls = '配货' 
	  if @@Rowcount = 0
	  begin
	    close c_edcS
	    deallocate c_edcS
		raiserror('在配货进货单根据修正链找不到原始单据，现在单号是%s',16,1,@num)
		return -1
	  end else
	  begin
	    insert into ShouldExchgDataDtl(SendDate,cls,num,checkint1,checkint2,checkdata1,tgt,src)
