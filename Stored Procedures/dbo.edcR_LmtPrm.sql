...
SET ANSI_NULLS ON
GO
create procedure [dbo].[edcR_LmtPrm] (
	@cls	varchar(30),
	@startdate	datetime,
	@finishdate	datetime
)
...
begin
  ...
  if @usergid = @zbgid return 0
	    ...
	    select convert(datetime,convert(char(10),p.fildate,102)), '限量促销单', p.srcnum, count(pd.line),
		@usergid,p.src,p.stat
	    from lmtprm p (nolock), lmtprmdtl pd (nolock)
