...
GO
/*
  将年报表中@store, @settleno, 的商品@oldgdgid, 改成@gdgid
*/
create procedure [dbo].[CombineGoodsYRpt]
	@store int,
	@settleno int,
	@oldgdgid int,  --被合并商品
	@gdgid int,     --并入商品
...
begin
	...
	begin
		...
		begin
			...
			begin
				update INYRPT set BGDGID = @gdgid
				where ASTORE = @store and ASETTLENO = @settleno
					and BGDGID = @oldgdgid and BVDRGID = @vdrgid and BWRH = @wrh
			end
			fetch next from c_rpt into
	...
	begin
		...
		open c_rpt
		fetch next from c_rpt into @vdrgid, @wrh,
			@DQ1, @DQ2, @DQ3, @DQ4, @DT1, @DT2, @DT3, @DT4,
			@DI1, @DI2, @DI3, @DI4, @DR1, @DR2, @DR3, @DR4
		while @@fetch_status = 0
		begin
	...
	begin
		...
		begin
			...
			begin
				update INVYRPT set BGDGID = @gdgid
				where ASTORE = @store and ASETTLENO = @settleno
					and BGDGID = @oldgdgid and BWRH = @wrh
			end
			fetch next from c_rpt into @wrh, @CQ, @CT, @FQ, @FT
	...
	begin
		...
		declare c_rpt cursor for
			...
			from INV 
			where STORE = @store and (GDGID = @oldgdgid or GDGID = @gdgid)
			group by WRH
		open c_rpt
		fetch next from c_rpt into @wrh, @FQ, @FT
		while @@fetch_status = 0
		begin
			if exists(select * from INVYRPT
				where ASTORE = @store and ASETTLENO = @settleno
	...
	begin
		...
		begin
			...
			begin
				update INVCHGYRPT set BGDGID = @gdgid
				where ASTORE = @store and ASETTLENO = @settleno
					and BGDGID = @oldgdgid and BWRH = @wrh
			end
			fetch next from c_rpt into
	...
	begin
		...
		begin
			...
			begin
				update OUTYRPT set BGDGID = @gdgid
				where ASTORE = @store and ASETTLENO = @settleno 
					and BGDGID = @oldgdgid and BCSTGID = @cstgid and BWRH = @wrh
			end
			fetch next from c_rpt into
	...
	begin	
		declare c_rpt cursor for
			select BCSTGID, BWRH, CQ1, CQ2, CQ3, CT1, CT2, CT3, CT4, 
		...
		begin
			...
			begin
				update CSTYRPT set BGDGID = @gdgid
				where ASTORE = @store and ASETTLENO = @settleno 
					and BGDGID = @oldgdgid and BCSTGID = @cstgid and BWRH = @wrh
			end
			fetch next from c_rpt into 
	...
	begin
		...
		declare c_rpt cursor for
			select BCSTGID, BWRH, SUM(DQ1), SUM(DQ2), SUM(DQ3), 
		...
		open c_rpt
		fetch next from c_rpt into 
			@cstgid, @wrh, @DQ1, @DQ2, @DQ3, @DT1, @DT2, @DT3
		while @@fetch_status = 0
		begin
			update CSTYRPT set
	...
	begin
		...
		begin
			...
			begin
				update VDRYRPT set BGDGID = @gdgid
				where ASTORE = @store and ASETTLENO = @settleno
					and BGDGID = @oldgdgid and BVDRGID = @vdrgid and BWRH = @wrh
			end
			fetch next from c_rpt into
