...
SET ANSI_NULLS ON
GO
create procedure [dbo].[PS3_BRAND_SENDONE]
(
  @piCode varchar(10),          --品牌代码
  @piRcvGid integer,            --接收门店
  @piOperGid integer,           --操作员
  @piFlag integer,              --是否已删除品牌资料，0-否；1-是
...
begin
  ...
  begin
    ...
    insert into NBrand(Src, ID, Code, Name, Rcv, Type, Area, TemplateGid, Creator, CreateTime,
      LstUpdOper, LstUpdTime, Note, IntroduceTime, IntroduceOper, BusinessStat, EliminateTime, BrandClass, FLAG)
    select @vSrc, @vID, Code, Name, @piRcvGid, 0, Area, TemplateGid, Creator, CreateTime,
      LstUpdOper, LstUpdTime, Note, IntroduceTime, IntroduceOper, BusinessStat, EliminateTime, BrandClass, 1
