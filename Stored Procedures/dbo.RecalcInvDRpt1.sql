...
SET ANSI_NULLS ON
GO
create procedure [dbo].[RecalcInvDRpt1]
	@store int,
	@settleno int,
	@date datetime
as
begin
    ...
    select #final.wrh, #final.gdgid, isnull(#init.qty, 0) cq, isnull(#init.total, 0) ct,
    	...
    	into #all
        from #final, #init, goodsh goods(nolock)
    ...
    declare c_prc cursor for
        ...
        from prcadj m(nolock), prcadjdtl d(nolock)
        where m.cls = d.cls and m.num = d.num
        and m.fildate >= @date and m.fildate <= dateadd(day, 1, @date)  /*原条件：and m.fildate <= dateadd(day, -1, @date)*/
    ...
    declare c_prc cursor for
        ...
        from rtlprcadj m(nolock), rtlprcadjdtl d(nolock)
        where  m.num = d.num
        and m.fildate >= @date and m.fildate <= dateadd(day, 1, @date)  /*原条件：and m.fildate <= dateadd(day, -1, @date)*/
        order by m.fildate asc
        for read only
