...
SET ANSI_NULLS ON
GO
create procedure [dbo].[edcS_StkOut] (
	@cls	varchar(30),
	@startdate	datetime,
	@finishdate		datetime
)
  ...
  begin
    if @stat = 1 
      insert into ShouldExchgDataDtl(SendDate,cls,num,checkint1,checkint2,checkdata1,tgt,src)
        values (convert(char(10), @fildate, 102),'配货出',@num,@reccnt,@stat,@checkdata1,@tgt,@usergid)
    ...
    begin
	  ...
	  from stkout o(nolock) where num = @modnum and stat = 2 and cls = '配货' 
	  if @@Rowcount = 0
	  begin
	    close c_edcS
	    deallocate c_edcS
		raiserror('在配货出货单根据修正链找不到原始单据，现在单号是%s',16,1,@num)
		return -1
	  end else
	    insert into ShouldExchgDataDtl(SendDate,cls,num,checkint1,checkint2,checkdata1,tgt,src)
	      values (convert(char(10), @fildate, 102),'配货出',@num,@reccnt,@stat,@checkdata1,@tgt,@usergid)
