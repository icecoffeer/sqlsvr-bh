...
SET ANSI_NULLS ON
GO
create procedure [dbo].[edcR_StkInBck] (
	@cls	varchar(30),
	@startdate	datetime,
	@finishdate	datetime
)
...
begin
  ...
  if @usergid <> @zbgid return 0
	...
	select convert(datetime,convert(varchar(10),o.fildate,102)),'配货进退',
	...
	begin
		if @stat = 1 
		    insert into RealExchgDataDtl(Recvdate,cls,num,checkint1,checkint2,tgt,src,checkdata1)
			values (convert(char(10), @fildate, 102),'配货进退',@srcnum,@reccnt,@stat,@usergid,@Src,@checkdata1)
		...
		begin
			...
			begin
				...
				from stkoutbck o(nolock) where num = @modnum and cls = '配货' and stat = 2 AND src = @src
			end
			else
			begin
				select @num = num,@srcnum = o.srcnum,@reccnt = o.reccnt,@modnum = o.modnum,
					@stat = o.stat,@checkdata1 = total
				from stkoutbck o(nolock) where srcnum = @modnum and cls = '配货' and stat = 2 AND src = @src
			end
			if @@Rowcount = 0
			begin
				select @num = num,@srcnum = o.srcnum,@reccnt = o.reccnt,@modnum = o.modnum,
				...
				begin
					close c_edcR
					Deallocate c_edcR
					Raiserror('在配货进货退货单中根据修正链找不到原始单据，现在单号是%s',16,1,@num)
					return -1
				end
			end
				--为了修正接收配货进退单过程的错误。
			    if not exists (select 1 from realexchgdatadtl
				where recvdate = @fildate and cls= '配货进退' 
