...
SET ANSI_NULLS ON
GO
create procedure [dbo].[OrderPool_Search_Goods](
...
begin
  ...
  set @vSQLStr = 'insert into ORDERPOOLTEMP(SPID, GDGID, VDRGID, WRH, COMBINETYPE, SENDDATE, QTY, PRICE, SPLITDAYS, ROUNDTYPE)
    select @@spid, ORDERPOOL.GDGID, ORDERPOOL.VDRGID, ORDERPOOL.WRH, ORDERPOOL.COMBINETYPE, ORDERPOOL.SENDDATE,
      sum(QTY), min(PRICE), max(SPLITDAYS), max(ROUNDTYPE)
    from ORDERPOOL(nolock)
      inner join GOODS(nolock) on ORDERPOOL.GDGID = GOODS.GID
    where 1=1
    and ORDERTYPE not like ''RF叫货申请'' '
  ...
  set @vSQLStr = @vSQLStr
    + ' group by ORDERPOOL.GDGID, ORDERPOOL.VDRGID, ORDERPOOL.WRH, ORDERPOOL.COMBINETYPE, ORDERPOOL.SENDDATE'
  exec(@vSQLStr)

  /*插入ORDERPOOLHTEMP表*/
  truncate table ORDERPOOLHTEMP
  set @vSQLStr = 'insert into ORDERPOOLHTEMP(SPID, UUID, GDGID, VDRGID, WRH, COMBINETYPE, SENDDATE,
    QTY, PRICE, ORDERTYPE, IMPTIME, IMPORTER, ORDERDATE, SPLITDAYS, NOTE, ROUNDTYPE)
    select @@spid, ORDERPOOL.UUID, ORDERPOOL.GDGID, ORDERPOOL.VDRGID, ORDERPOOL.WRH, ORDERPOOL.COMBINETYPE, ORDERPOOL.SENDDATE,
      ORDERPOOL.QTY, ORDERPOOL.PRICE, ORDERPOOL.ORDERTYPE, ORDERPOOL.IMPTIME, ORDERPOOL.IMPORTER, ORDERPOOL.ORDERDATE, ORDERPOOL.SPLITDAYS, ORDERPOOL.NOTE, ORDERPOOL.ROUNDTYPE
    from ORDERPOOL(nolock)
      inner join GOODS(nolock) on ORDERPOOL.GDGID = GOODS.GID
    where 1=1
    and ORDERTYPE not like ''RF叫货申请'' '
