...
SET ANSI_NULLS ON
GO
create procedure [dbo].[PCT_CHGBOOK_CALC_RATETOTAL2] (
...
begin
  ...
  select @vRowNo = min(ROWNO)
  from CTCNTRRATESTOREGDDISC where NUM = @piCntrNum and VERSION = @piCntrVersion and LINE = @piCntrLine 
    and ITEMNO = @piItemNo and HIGHAMT > @piBaseTotal
  if @vRowNo is null
  begin
    select @vRowNo = max(ROWNO)
    from CTCNTRRATESTOREGDDISC where NUM = @piCntrNum and VERSION = @piCntrVersion and LINE = @piCntrLine and ITEMNO = @piItemNo
    if @vRowNo is null
    begin
  ...
  select 
    ...
    @vQBase = QBASE
  from CTCNTRRATESTOREGDDISC where NUM = @piCntrNum and VERSION = @piCntrVersion and LINE = @piCntrLine 
    and ITEMNO = @piItemNo and ROWNO = @vRowNo
  ...
  exec PCT_CHGBOOK_LOGDEBUG 'Calc_RateTotal2', @vMessage
