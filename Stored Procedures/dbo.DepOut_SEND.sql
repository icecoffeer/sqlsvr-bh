...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[DepOut_SEND]
(
  @PINUM VARCHAR(14),
...
BEGIN
  ...
  BEGIN  
    ...
    UPDATE CNTRDPTBILL SET SNDTIME = GETDATE() WHERE NUM = @PINUM AND CLS = '付'
  ...
  IF EXISTS(SELECT 1 FROM CNTRDPTOUTSRCDTL WHERE NUM = @PINUM)
  BEGIN
  --生成网络压库金额付款单来源明细
    INSERT INTO NCNTRDPTOUTSRCDTL(NUM, LINE, SRCCLS, SRCNUM, SRCTOTAL, PAYEDAMT, CANPAYAMT,
      TOTAL, NOTE, ID, SRC)
    SELECT NUM, LINE, SRCCLS, SRCNUM, SRCTOTAL, PAYEDAMT, CANPAYAMT, TOTAL, NOTE, @ID, @SRC
     FROM CNTRDPTOUTSRCDTL
    WHERE NUM = @PINUM
  END
