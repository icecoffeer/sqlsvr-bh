...
SET ANSI_NULLS OFF
GO
CREATE Procedure [dbo].[TA_CVDRPAYM]
...
FROM GOODSH GOODSH(NOLOCK), VENDOR VENDOR(NOLOCK), zhcheckdtl zhcheckdtl(NOLOCK), ZHCHECK ZHCHECK(NOLOCK)
WHERE GOODSH.BILLTO = VENDOR.GID
 and  GOODSH.GID = zhcheckdtl.gdgid
-- and  DEPT.CODE = GOODSH.F1 
 AND ZHCHECKDTL.NUM=ZHCHECK.NUM
 and  GOODSH.sort LIKE ''x01%'' 
