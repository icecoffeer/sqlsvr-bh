...
GO
/*
  将日报表中@store, @settleno, 的商品@oldwrh, 改成@wrh
*/
create procedure [dbo].[CombineWrhDRpt]
  @store int,
  @settleno int,
  @oldwrh int,  --被合并仓位
  @wrh int      --并入仓位
...
begin
  ...
  /*INVDRPT*/
  declare c_rpt cursor for
    select ADATE, BGDGID, CQ, CT, FQ, FT
    from INVDRPT
    where ASTORE = @store and ASETTLENO = @settleno
      and BWRH = @oldwrh
  open c_rpt
  fetch next from c_rpt into @ADATE, @gdgid, @CQ, @CT, @FQ, @FT
  while @@fetch_status = 0
  begin
  ...
  /* CSTDRPT */
  declare c_rpt cursor for
    select ADATE, BCSTGID, BGDGID, DQ1, DQ2, DQ3, DT1, DT2, DT3
    from CSTDRPT
    where ASTORE = @store and ASETTLENO = @settleno
      and BWRH = @oldwrh
  open c_rpt
  fetch next from c_rpt into
    @ADATE, @cstgid, @gdgid, @DQ1, @DQ2, @DQ3, @DT1, @DT2, @DT3
  while @@fetch_status = 0
  begin
    ...
    fetch next from c_rpt into
      @ADATE, @cstgid, @gdgid, @DQ1, @DQ2, @DQ3,
      @DT1, @DT2, @DT3
  end
  close c_rpt
  deallocate c_rpt
