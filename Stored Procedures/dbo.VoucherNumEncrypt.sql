...
create proc [dbo].[VoucherNumEncrypt]
(
...
begin
  ...
  set @poENSN = '';
  set @vch = '';
  select @kdSn = REVERSE(RTRIM(USERID)) FROM FASYSTEM(NOLOCK);
  if @kdsn <> ''
 ...
 --------------------------------------------------------
  ...
  -- @poENSN 为左移的结果
  select @poEnsn = replace(@poEnsn, 'A', 0);
  select @poEnsn = replace(@poEnsn, 'B', 2);
  select @poEnsn = replace(@poEnsn, 'C', 1);
  select @poEnsn = replace(@poEnsn, 'D', 6);
  select @poEnsn = replace(@poEnsn, 'E', 3);
  select @poEnsn = replace(@poEnsn, 'F', 7);
  return (@vRnt);
end
GO
