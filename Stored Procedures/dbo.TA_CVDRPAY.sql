...
CREATE Procedure [dbo].[TA_CVDRPAY]
...
EXEC ('DECLARE CUR_GETVOUCHER CURSOR GLOBAL FOR 
	...
	, VENDOR.NAME  C
	, convert(VARCHAR(100),SUM(zhcheckdtl.dxouttotal-zhcheckdtl.payinvqty*zhcheckdtl.rtlprc)) D
        , convert(varchar(100),sum((zhcheckdtl.dxoutintotal-zhcheckdtl.payinvqty*zhcheckdtl.inprc)/1.17*0.17)) E
	,convert(varchar(100),sum(zhcheckdtl.dxoutintotal-zhcheckdtl.payinvqty*zhcheckdtl.inprc))F
        ,convert(varchar(100),sum(zhcheckdtl.dxouttotal-zhcheckdtl.payinvqty*zhcheckdtl.rtlprc-(zhcheckdtl.dxoutintotal-zhcheckdtl.payinvqty*zhcheckdtl.inprc)/1.17)) G
