...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[VoucherReceiveWithStartEndNum]
(
    @num varchar(32),
    @endNum varchar(32),
    @storeGid int,
...
BEGIN
    ...
    BEGIN
        EXEC VoucherReceiveSingle @num, @storeGid, @operator, @errorMessage OUTPUT;
    ...
    -- 回收最后一张
    IF @num IS NOT NULL EXEC VoucherReceiveSingle @num, @storeGid, @operator, @errorMessage OUTPUT;
    IF @errorMessage IS NOT NULL
    BEGIN
      -- 生成时发生了错误
      -- 目前项目上要求发生错误就终止
