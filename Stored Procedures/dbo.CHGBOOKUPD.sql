...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[CHGBOOKUPD]
(
  @NUM 	CHAR(14),
  @OPER	CHAR(30),
...
BEGIN
	...
	SELECT 
		...
		--@SNDTIME = SNDTIME		
	FROM CHGBOOK WHERE NUM = @NUM
	SELECT @VSTAT = STAT, @SNDTIME = SNDTIME --应该是根据被修正单据的发送时间来判断 zz 090402
	FROM CHGBOOK WHERE NUM = @VMODNUM
	IF @VSTAT <> 500
	BEGIN
	...
	WHERE IVCCODE = @VMODNUM AND CHGTYPE = '费用单'
	--修改所有引用当前费用单的交款单的记录
	UPDATE VDRPAYDTL SET CHGNUM = @NUM
	WHERE CHGNUM = @VMODNUM

	--修改被修正单据状态     
	UPDATE CHGBOOK SET STAT = 530, LSTUPDTIME = GETDATE()
	WHERE NUM = @VMODNUM
	--Fanduoyi 1298
	INSERT INTO CHGBOOKLOG(num, stat, modifier, time)
		VALUES(@VMODNUM, 530, @oper, getdate())
	
	EXEC @VRET = CHKCHGBOOKTO500 @NUM, @OPER, 'NOSEND', @MSG OUTPUT
