...
begin
	declare @d_total money, @sql varchar(255)
	
	select @d_total = sum(QTY * PRICE)
		from rb_BUY2
		where POSNO = @p_posno and FLOWNO = @p_flowno
	if @d_total is null return(0)
	
	begin transaction
	select @sql = 'update ' + @p_database + '..BUY1_' + @p_posno
		+ ' set TOTAL = ' + convert(varchar, @d_total, 2) + ', TAG = 0'
		+ ' where POSNO = ''' + @p_posno + ''' and FLOWNO = ''' + @p_flowno + ''''
	exec (@sql)
	commit transaction
