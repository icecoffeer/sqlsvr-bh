...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[CHKCHGBOOKTO500]  
(  
   @PINUM   CHAR(14),  
   @PIOPER  CHAR(30),  
   @PICLS      CHAR(10),  
...
begin  
 ...
 IF @@ROWCOUNT = 0  
 BEGIN  
  SET @POERR_MSG = '费用单' + @PINUM + '不存在'  
  RETURN 1  
 END  
  
  if isnull(@vCntrNum, '') <> '' and (@SRC is null or @SRC = @USERGID) --有可能合约并没有发送到结算中心
  begin  
 ...
 BEGIN  
  ...
  BEGIN  
   SET @POERR_MSG = '费用单' + @PINUM + '中的对应的帐款项目' + @Vchgcode + '不存在。'  
   RETURN 1  
  END  
 END  
 ...
 IF @VISVALID = 1  
 BEGIN  
  SET @POERR_MSG = '费用单' + @PINUM + '中的对应的项目' + @VCHGCODE + '已经无效。';  
     RETURN 1  
 END  
  
 SELECT @SETTLENO = MAX(NO) FROM MONTHSETTLE(NOLOCK)
 SET @SRC = NULL  
 SELECT @SRC = C2.SRC FROM CHGBOOK C1(NOLOCK), CHGBOOK C2(NOLOCK) WHERE C1.NUM = @PINUM AND C1.MODNUM = C2.NUM  
 ...
 UPDATE CHGBOOK SET SRC = @USERGID WHERE NUM = @PINUM AND SRC IS NULL  
