...
SET ANSI_NULLS ON
GO
create procedure [dbo].[PRMOFFSET_GetFromTempTable](
  @Filler int,
...
begin
  ...
  if @Opt_CSTMINSERT <> 1
  begin
  	set @Msg = '选项CSTMINSERT(727)的值为' + convert(varchar, @Opt_CSTMINSERT) + '，不支持收入录入及xls文件导入。'
  	return 1
  end
  ...
  begin
    ...
    begin
      ...
      VALUES (@Num, @VdrGid, @SettleNo, getdate(), @Filler, getdate(), 0, @UserGid, 0,
        '', 1, @VdrGid, @Opt_OFFSETTYPE, @Opt_OFFSETCALCTYPE, @Opt_GatheringMode,
    ...
    --明细
		...
		INSERT INTO PrmOffsetDtl(Num, Line, SettleNo, GDGid, Qpc, QpcStr,
		  AGMNUM, AGMLINE, SAMT, RAMT, OffsetPrc, CntInPrc, Qty,
 		  Start, Finish, Note, Alc, Total, Tax, Amount, DiffPrc,
 		  PAYQTY, PAYAMT, SQTY, AGMTABLENAME)
    VALUES (@Num, @Line, @SettleNo, @GdGid, 1, '1*1',
      @AgmNum, @AgmLine, @Ramt, @Ramt, @Offsetprc, @CntInPrc, @Qty,
      convert(varchar(10), getdate(), 121), convert(varchar(10), getdate() + 1, 121), @Note,
      @Alc, @Total, @Tax, @Amount, @DiffPrc,
      0, 0, @Qty, 'PRMOFFSETAGM')

    --明细2

    insert into PRMOFFSETDTLDTL(NUM, LINE, ITEM, GDGID, STOREGID, AGMNUM, AGMLINE, SAMT, RAMT, SQTY, RQTY, AGMTABLENAME)
      select @Num, @Line, 1, @GdGid, @UserGid, @AgmNum, @AgmLine, @Ramt, @Ramt, @Qty, @Qty, 'PRMOFFSETAGM'
