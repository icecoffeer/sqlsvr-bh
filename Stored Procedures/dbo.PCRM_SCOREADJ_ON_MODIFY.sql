...
SET ANSI_NULLS OFF
GO
create procedure [dbo].[PCRM_SCOREADJ_ON_MODIFY]
(
  @piNum varchar(14),
  @piToStat int,
  @piOperGid int,
...
begin
  ...
  begin
    set @poErrMsg = '该单据状态: ' + rtrim(convert(varchar, @piToStat)) + ' 不存在!'
    return(1)
  end
  ...
  begin
    exec @vRet = PCRM_SCOREADJ_STAT_TO_100 @piNum, @piOperGid, @poErrMsg output
    if (@vRet <> 0) or (@piToStat = 100)
      return(@vRet)
  end
  if (@piToStat = 300) and (@vStat in (0, 100))
  begin
    exec @vRet = PCRM_SCOREADJ_STAT_TO_300 @piNum, @piOperGid, @poErrMsg output
    return(@vRet)
  end
  
  select @vOper = rtrim(NAME) + '[' + rtrim(CODE) + ']' from EMPLOYEE(nolock) where GID = @piOperGid
  update CRMSCOREADJ set MODIFIER = @vOper, LSTUPDTIME = getdate() where NUM = @piNum
  
  exec PCRM_SCOREADJ_ADD_LOG @piNum, @vStat, @piToStat, @piOperGid
  return(0)
end
GO
