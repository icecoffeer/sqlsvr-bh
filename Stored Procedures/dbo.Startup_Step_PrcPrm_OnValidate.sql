...
SET ANSI_NULLS ON
GO
Create Procedure [dbo].[Startup_Step_PrcPrm_OnValidate](
  @num varchar(10)
) as
begin
  ...
  --触发的数据字典dict
  execute PFA_SERIALIZEXML_SETSTRING 'NUM', @num, @xml_num output
  set @xml_map_dict=@xml_num

  --模块参数  
  execute PFA_GET_OPERINFO_BYGID @usergid output, @usercode output, @Username output
  set @xml_title = '促销单[' + @num + ']在' + Convert(varchar, @applydate, 20) + '生效了。'
  ...
  --插入触发记录
  execute PFA_MscbNotify_AppendNotify 'PS3_HDBasic_PrcPrmDetail', '促销单生效提醒', @xml_map_dict, @xml_map_prompt, @applydate, @usergid, @usercode, @username, @return output
end
GO
