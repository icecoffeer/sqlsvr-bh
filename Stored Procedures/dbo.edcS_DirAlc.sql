...
SET ANSI_NULLS ON
GO
create procedure [dbo].[edcS_DirAlc] (
	@cls	varchar(30),
	@startdate	datetime,
	@finishdate		datetime
)
...
begin
  ...
  if (@cls in ('直配进', '直配进退') and @usergid = @zbgid)
	...
	select convert(datetime,convert(varchar(10),o.fildate,102)),
		o.num,o.reccnt,o.total,o.modnum,o.stat,
		case when o.cls in ('直配进', '直配进退') then o.sender else o.receiver end
		from diralc o (nolock) 
	where o.stat in (1,4) 
    ...
    begin
		if @stat = 1 
			...
			values (convert(char(10), @fildate, 102),@cls,@num,@reccnt,@stat,@checkdata1,@tgt,@usergid)
		
		while isnull(@ModNum,'') <> ''
		begin
			select @num = '',@reccnt = 0
			select @num = o.num,@reccnt = o.reccnt,@modnum = o.modnum,
				@stat = o.stat,@checkdata1 = o.total,
				@tgt = o.receiver
			from diralc o(nolock) where num = @modnum and stat = 2 and cls = @cls
			if @@RowCount = 0
			begin
				close c_edcS
				deallocate c_edcS
				raiserror('在%s单根据修正链找不到原始单据，现在单号是%s',16,1,@cls,@num)
				return -1
			end
			else
  			    insert into ShouldExchgDataDtl(SendDate,cls,num,checkint1,checkint2,checkdata1,tgt,src)
