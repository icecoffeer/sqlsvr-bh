...
SET ANSI_NULLS ON
GO
create procedure [dbo].[RcvInvChgMRpt] as
begin
  declare
  ...
  begin
    ...
    if @BGDGID is null
    begin
      update NINVCHGMRPT set NSTAT = 1, NNOTE = '商品不存在'
         where NINVCHGMRPT.ID = @ID
      fetch next from c_inVCHGMrpt into
          @id,@astore,@asettleno,@BGDGID,@bwrh,
          @DQ1,@DQ2,@DQ4,@DQ5,@DQ6,@DQ7,@DT6,@DT7,@DI1,@DI2,@DI3,@DI4,@DI5,@DI6,@DI7,
          @DR1,@DR2,@DR3,@DR4,@DR5,@DR6,@DR7,
    ...
    (@ASTORE,@ASETTLENO,@BGDGID,@BWRH,
     ...
     @CR1,@CR2,@CR3,@CR4,@CR5,@CR6,@CR7, @DI8) --modified by wang xin 2003.02.17
    /* 删除NINVCHGMRPT */
    delete from NINVCHGMRPT where ID = @ID
    commit transaction
