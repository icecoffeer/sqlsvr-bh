...
SET ANSI_NULLS ON
GO
create procedure [dbo].[HDCREATEINDEX]
(
...
begin
  ...
  begin
    ...
    begin
      if not exists (select 1 from SYSOBJECTS a, SYSOBJECTS b where a.PARENT_OBJ = b.ID
      ...
      begin
        ...
        print  'WARNING: 增加约束' + @piTableName + '.' + @piIndexName + '失败，约束已存在'
    end else
      if not exists(select 1 from SYSINDEXES i, SYSOBJECTS o where o.NAME = @piTableName 
      ...
      begin
        ...
        print 'WARNING: 创建索引' + @piTableName + '.' + @piIndexName + '失败，索引已存在'
  end    
  return(0)
end
GO
