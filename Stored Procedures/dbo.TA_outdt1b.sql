...
CREATE Procedure [dbo].[TA_outdt1b]
...
EXEC ('DECLARE CUR_GETVOUCHER CURSOR GLOBAL FOR 
	...
	convert(int,GOODSH.SALETAX) O
      FROM OUTDRPT OUTDRPT(NOLOCK), GOODSH GOODSH(NOLOCK), V_GDSORTB V_GDSORTB(NOLOCK), 
           STORE STORE(NOLOCK)
      WHERE OUTDRPT.ASTORE = STORE.GID
      and OUTDRPT.BGDGID = V_GDSORTB.GID
...
--      and convert(int,GOODSH.SALETAX)<>5
	 ...
	 'GROUP BY STORE.NAME, STORE.CODE, V_GDSORTB.SORT, V_GDSORTB.NAME,GOODSH.SALETAX')
