...
SET ANSI_NULLS ON
GO
Create Procedure [dbo].[PrcAdjChkValidate](
  @p_cls varchar(8),
  @p_num varchar(10)
) as
begin
  ...
  set @xml_title = RTRIM(Convert(char, @p_cls)) + '调整单[' + @p_num + ']在' + Convert(varchar, @applydate, 20) + '生效了'
  ...
  --插入触发记录
  set @xml_event= RTRIM(Convert(char, @p_cls)) + '生效提醒'
