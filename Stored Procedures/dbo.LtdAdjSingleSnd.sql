...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[LtdAdjSingleSnd] (
  @NUM  CHAR(14),
  @STORE INT,
  @OPER INT
) AS
BEGIN
  ...
  IF (@USERGID = @STORE)
  BEGIN
    RAISERROR('不能发送给自己', 16, 1)
    RETURN 1
  END

  SELECT @TAG = 0
  IF EXISTS(SELECT 1 FROM LTDADJLACDTL WHERE NUM = @NUM AND STOREGID = @STORE)
        SET @TAG = 1
  ELSE IF EXISTS(SELECT 1 FROM LTDADJLACDTL2 WHERE NUM = @NUM AND ALCGID = @STORE)
    SET @TAG = 2

  IF (@TAG = 0)
  BEGIN
    RAISERROR('输入的目标店，不是该单据的生效门店或配送中心', 16, 1)
    RETURN 1
  END


  /*发送到生效配送中心*/
  IF (@STORE <> @USERGID) AND (@TAG = 2)
  BEGIN
    ...
    @LAUNCH, @USERGID, GETDATE(), @STORE, NULL, 0, 0, NULL, @KEEPATYPE)
  ...
  BEGIN
    ...
    @LAUNCH, @USERGID, GETDATE(), @STORE, NULL, 0, 0, NULL, @KEEPATYPE)
      ...
      SELECT @USERGID, @BILLID, LINE, GDGID, OLDVALUE, NEWVALUE, NOTE, OLDSALE, NEWSALE, FINISHDATE
      FROM LTDADJDTL WHERE NUM = @NUM
      IF NOT EXISTS(SELECT 1 FROM LTDADJLACDTL2 WHERE NUM = @NUM AND ALCGID = @STORE) --如果该门店不在生效配送中心中，则需要发送生效单位明细，记录只有一条，就是该门店
        INSERT INTO NLTDADJLACDTL(SRC, ID, NUM, STOREGID) VALUES(@USERGID, @BILLID, @NUM, @STORE)
  END
