...
SET ANSI_NULLS ON
GO
create procedure [dbo].[PCT_VDRPAYRTN_ADDLOG] (
  @piNum varchar(14),
  @piFromStat integer,
  @piToStat integer,
  @piOperGid integer
...
begin
  ...
  select @vItemNo = isnull(max(ITEMNO), 0) from CTVDRPAYRTNLOG(nolock) where NUM = @piNum
  set @vItemNo = @vItemNo + 1
  insert into CTVDRPAYRTNLOG(NUM, ITEMNO, FROMSTAT, TOSTAT, OPER, OPERTIME)
